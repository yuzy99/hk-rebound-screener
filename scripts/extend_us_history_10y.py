...
from hk_rebound_screener.adapters import (  # noqa: E402
    fetch_us_common_stock_master,
    fetch_yfinance_history_bulk,
)
from hk_rebound_screener.strategy import normalize_code  # noqa: E402


CACHE_DIR = PROJECT_ROOT / "data" / "yfinance_cache_10y"
HISTORY_FILE = CACHE_DIR / "us_history.csv"
QUALITY_FILE = CACHE_DIR / "us_history_quality.json"
PROTECTED_FILE = PROJECT_ROOT / "data" / "yfinance_cache" / "us_history.csv"
REQUIRED_COLUMNS = [
    "date",
    "code",
    "open",
...
def read_codes(path: Path) -> set[str]:
    if not path.exists():
        return set()
    frame = pd.read_csv(path, usecols=["code"], dtype={"code": "string"})
    return {
        normalize_code(code)
...
def write_quality_manifest(
    ...
    requested_end: date,
    protected_sha_before: str | None,
    protected_sha_after: str | None,
) -> dict:
    frame = frame.copy()
    ...
    quality = {
        "dataset": "US historical OHLCV cache (separate 10-year cache)",
        ...
        "path_from_repository_root": "data/yfinance_cache_10y/us_history.csv",
        "source": "yfinance",
        "encoding": "UTF-8",
        "bom": False,
        "delimiter": ",",
        ...
        "quality_checks": {
            ...
            "invalid_codes": int((~frame["code"].fillna("").str.fullmatch(r"[A-Z][A-Z0-9.\-]*")).sum()),
            ...
            "non_finite_numeric_cells": int((~np.isfinite(frame[NUMERIC_COLUMNS])).sum().sum()),
        ...
        "coverage_notes": {
            ...
            "latest_date_code_count": int(date_counts.get(latest_date, 0)) if latest_date is not None else 0,
            ...
            "requested_missing_sample": sorted(requested_codes - actual_codes)[:50],
            "current_us_master_codes": len(master_codes),
            "master_codes_without_history": len(master_codes - actual_codes),
            "master_missing_sample": sorted(master_codes - actual_codes)[:50],
            "history_codes_not_in_current_master": len(actual_codes - master_codes),
            "history_not_in_master_sample": sorted(actual_codes - master_codes)[:50],
        ...
        "cache_isolation": {
            "active_cache": "data/yfinance_cache_10y/us_history.csv",
            "protected_existing_cache": "data/yfinance_cache/us_history.csv",
            "protected_sha256_before": protected_sha_before,
            "protected_sha256_after": protected_sha_after,
            "protected_existing_cache_unchanged": protected_sha_before == protected_sha_after,
        },
        "use_boundary": {
        ...
        "related_files": {
            "current_four_year_cache": "data/yfinance_cache/us_history.csv",
            "quality_manifest": "data/yfinance_cache_10y/us_history_quality.json",
        },
    }
...
def main() -> int:
    today = date.today()
    parser = argparse.ArgumentParser(description="在独立目录增量补齐美股 yfinance 十年日线缓存")
    ...
    protected_sha_before = sha256(PROTECTED_FILE)
    old_codes = read_codes(PROTECTED_FILE)
    separate_codes = read_codes(HISTORY_FILE)
    try:
    ...
    except Exception as error:  # noqa: BLE001 - existing lists remain a safe fallback
        master_codes = set()
        print(f"WARN 当前美股主表读取失败，将沿用已有代码: {error}", flush=True)

    requested_codes = old_codes | separate_codes | master_codes
    if not requested_codes:
        raise RuntimeError("没有可下载的美股代码")
    print(
        f"美股独立十年缓存: {args.start} 至 {args.end}；现用缓存代码 {len(old_codes)}，"
        f"独立缓存代码 {len(separate_codes)}，当前主表 {len(master_codes)}，请求并集 {len(requested_codes)}",
        flush=True,
    )
    ...
    if not HISTORY_FILE.exists():
        raise RuntimeError("下载结束但独立美股历史文件不存在")

    protected_sha_after = sha256(PROTECTED_FILE)
    ...
    print(
        f"美股独立十年缓存完成: {quality['rows']:,} 行，{quality['unique_codes']:,} 只代码，"
    ...
    print(
        ...
        f"现用四年文件未改变 {protected_sha_before == protected_sha_after}",
        flush=True,
    )
    return 0
